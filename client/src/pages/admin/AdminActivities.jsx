...
export default function AdminActivities() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError('');
      try {
        const { data } = await api.get('/admin/popular-activities');
        if (!cancelled) {
        ...
        <div className="mx-auto max-w-7xl">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-stone-400">
                  {activities.length} activities sorted by usage
                </p>
              </div>
            </div>
          </motion.div>
          ...
          {error ? (
            ...
            <div className="mt-12 rounded-2xl border border-white/10 bg-white/5 p-10 text-center">
              <Activity className="mx-auto mb-3 h-10 w-10 text-stone-500" />
              <p className="text-sm text-stone-400">No activities found.</p>
            </div>
          ) : (
            <motion.div
            ...
            >
              <div className="overflow-x-auto">
                <table className="w-full min-w-[550px] text-left text-sm">
                  ...
                  <tbody>
                    {activities.map((act) => {
                      const TypeIcon = TYPE_ICON[act.type] || Activity;
                      return (
                        <tr key={act.id} className="border-b border-white/5 transition hover:bg-white/5">
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-2">
                              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-violet-400/20 text-violet-300">
                                <TypeIcon className="h-3.5 w-3.5" />
                              </div>
                              <span className="font-medium text-white">{act.name}</span>
                            </div>
                          </td>
                          <td className="px-5 py-3.5 text-stone-300">{act.cityName || '—'}</td>
                          <td className="px-5 py-3.5">
                            <span
                              className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.1em] ${typeBadgeColor(act.type)}`}
                            >
                              {act.type || '—'}
                            </span>
                          </td>
